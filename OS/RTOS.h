...
/*********************************************************************
*
*     Chip specifics for Cortex M cores and GNU compiler
*
**********************************************************************
*/
...
/*****  End of configuration settings *******************************/
...
#if (defined __ARM_ARCH_6M__) || (defined __ARM_ARCH_8M_BASE__)                                                                 // Cortex-M0/M0+/M1, ARMv8-M baseline
...
#elif (defined __ARM_ARCH_7M__) || ((defined __ARM_ARCH_7EM__) && (USE_ERRATUM_837070 == 0)) || (defined __ARM_ARCH_8M_MAIN__)  // Cortex-M3, Cortex-M4/M4F/M7 w/o workaround, ARMv8-M mainline
  //
  // The NOPs should flush the pipeline to ensure that interrupts
  // are disabled before the next instruction is executed.
  //
  #define OS_INT_Disable() { __asm volatile ("movs r0, #128    \n\t"  \
                                    "msr  basepri, r0 \n\t"  \
                                    "nop              \n\t"  \
                                    "nop              \n\t"  \
                                    :                        \
  ...
  #define OS_INT_Enable() { __asm volatile ("movs r0, #0      \n\t"  \
                                    "msr  basepri, r0 \n\t"  \
                                    :                        \
                                    :                        \
                                    : "r0"                   \
...
#elif ((defined __ARM_ARCH_7EM__) && (USE_ERRATUM_837070 == 1))                                                                 // Cortex-M4/M4F/M7 with workaround
  //
  // The NOPs should flush the pipeline to ensure that interrupts
  // are disabled before the next instruction is executed.
  //
...
#if (defined (__VFP_FP__) && !defined (__SOFTFP__)) || (defined __VFP_HARD__)
...
*/
void OS_HeapLock     (void);
void OS_HeapUnlock   (void);
void OS_PrintfLock   (void);
void OS_PrintfUnlock (void);
void OS_ScanfLock    (void);
void OS_ScanfUnlock  (void);
void OS_DebugIOLock  (void);
void OS_DebugIOUnlock(void);
...
void OS_TASK_SetContextExtensionTLS(void);  // Initialize Thread Local Storage, OS_TLS.c, CPU specific
//
...
#define OS_INT_PreserveAndDisableAll(p) { __asm volatile ("mrs  r1, primask \n\t"  \
                                                          "str  r1, [%0]    \n\t"  \
                                                          "cpsid i          \n\t"  \
                                                          :                        \
                                                          : "l" (p)                \
...
*/
#define OS_DATA_MEMORY_BARRIER()  __sync_synchronize();  // GCC built-in function to issue a full memory barrier
...
// embOS version
//
#define OS_VERSION_GENERIC    (50201u)
...
// OS generic ************************************
  ...
  OS_ERR_STACK                           = (120u),  // Task stack overflow or invalid task stack.
...
// Not matching routine calls or macro usage
  OS_ERR_UNUSE_BEFORE_USE                = (150u),  // OS_MUTEX_Unlock() has been called on a mutex hasn't been locked before.
  ...
  OS_ERR_RESOURCE_OWNER                  = (156u),  // OS_MUTEX_Unlock() has been called from a task which does not own the mutex.
...
// The first 32 SyetemView IDs are generic IDs.
...
struct OS_TASK_STRUCT {
  ...
  OS_PRIO              Priority;
  OS_PRIO              BasePrio;
  OS_TASKEVENT         Events;          // Event storage
...
#define  OS_MUTEX_CREATE(ps)  OS_MUTEX_Create(ps)
...
//
// For some compiler (e.g. IAR EWRX with optimization) it is necessary to use an OS_TASK_EnterRegion() function
// instead of the macro for OS_INT_EnterNestable(). To do so please add the following line to OSCHIP.h:
...
/*********************************************************************
*
*        usec precise system time
*
**********************************************************************
*/
#ifdef OS_U64
void   OS_TIME_ConfigSysTimer(const OS_SYSTIMER_CONFIG* pConfig) OS_CODE_SECTION_ATTRIBUTE;                      // Routine for setting up values for OS_TIME_Getus()
OS_U32 OS_TIME_Getus    (void)                                   OS_CODE_SECTION_ATTRIBUTE OS_SECURE_ATTRIBUTE;  // Routine for calculating current system time in usec
...
#define OS_GetTimeSliceReload                                                 OS_TASK_GetTimeSliceReload
...
#define OS_SetFastIntPriorityLimit                                            OS_INT_SetPriorityThreshold
...
#define OS_SendStringFast                                                     OS_COM_SendStringFast
...
#define OS_EvaPacketEx                                                        OS_COM_EvaPacketEx
